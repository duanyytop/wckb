                    ...
                    _tab == Tab.Create
                        ? (_showMnemonic
                            ...
                            : createWalletWidget(context, () {
                                if (_name.isEmpty || _tempPassword.isEmpty || _tempConfirmPassword.isEmpty) {
                                  Toast.show("Wallet name and password must be not empty", context,
                                      duration: Toast.LENGTH_SHORT, gravity: Toast.CENTER);
                                } else if (_tempPassword.length < 8) {
...
Widget importMnemonicWidget(BuildContext context) {
  return Column(
    children: <Widget>[
      Padding(
...
Widget inputWidget(String title, bool isPassword, Function onChanged, {bool isMnemonic = false}) {
  return Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          TextField(
            ...
            keyboardType: isMnemonic ? TextInputType.multiline : TextInputType.text,
            maxLines: isMnemonic ? null : 1,
...
Future<void> _saveWallet(BuildContext context) async {
  if (_name.isEmpty || _tempPassword.isEmpty) {
    Toast.show("Wallet name and password must be not empty", context,
        duration: Toast.LENGTH_SHORT, gravity: Toast.CENTER);
  } else if (_tempPassword.length < 8) {
    Toast.show("Password length must be bigger than 8", context, duration: Toast.LENGTH_SHORT, gravity: Toast.CENTER);
  } else {
    var _wallet = await importWalletWithMnemonic(_mnemonic, _tempPassword, _name);
    var result = await _wallet.save();
    if (result) {
      Navigator.push(context, MaterialPageRoute(builder: (context) => Transfer()));
    } else {
      Toast.show("Wallet save error", context, duration: Toast.LENGTH_SHORT, gravity: Toast.CENTER);
    }
  }
}
